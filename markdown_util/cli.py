...
语义一致（命名规则、双目录冲突检查同 :mod:`server.media_server`），
但无需服务器在线——直接文件系统 + sqlite 操作。

用法（仓库根目录）::

    python -m markdown_util import-media <路径...> [--kb-root DIR] [--asset] [--dry-run]

    # 或在 markdown_util 目录下直接
    python cli.py import-media ../some/*.png
...
if __package__ in (None, ""):  # python cli.py 直跑时把包目录挂上，走平铺导入
