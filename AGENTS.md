...
mdtool 是个人 Markdown 知识库管理器 + 工具集，三端分工：桌面 PySide6 管理器（mdtool/desktop）+ Typora 专职编辑 + 安卓只读阅读器（android）。改代码前先读 [docs/定位与架构分析.md](docs/定位与架构分析.md)（分工与路线图）与 [docs/知识库规范.md](docs/知识库规范.md)（链接模型与目录布局）；安卓端另见 [docs/安卓端规划.md](docs/安卓端规划.md)，Hexo 博客源接管（第三种知识库形态）见 [docs/博客文章管理规划.md](docs/博客文章管理规划.md)。
...
docs/            定位/架构、知识库规范、安卓端规划、博客文章管理规划、博客工作流——规范以这里为唯一权威
...
- **媒体链接必须经 resolver**：存储形式 = `http://127.0.0.1:8765/images|assets/<name>` 绝对 URL；任何消费方（zip 导出、手机渲染）先 `parse` → `rewrite`，非本库 URL 一律原样透传。`mdtool/core/link_resolver.py` 与 `android/.../resolver/LinkResolver.kt` 同语义，改一端必同步另一端及其测试。例外：`hexo` 目标（博客形态相对链接）仅桌面管理侧使用，手机端不消费博客形态，不同步（见 docs/博客文章管理规划.md）。
