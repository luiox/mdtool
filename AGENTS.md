...
mdtool 是个人 Markdown 知识库管理器 + 工具集，三端分工：桌面 PySide6 管理器（markdown_util）+ Typora 专职编辑 + 安卓只读阅读器（android）。改代码前先读 [docs/定位与架构分析.md](docs/定位与架构分析.md)（分工与路线图）与 [docs/知识库规范.md](docs/知识库规范.md)（链接模型与目录布局）；安卓端另见 [docs/安卓端规划.md](docs/安卓端规划.md)。

## Repository layout

```
markdown_util/   PySide6 桌面管理器；唯一 Qt 入口 main_qt.py（qtui/ 界面层，server/ 媒体服务器与 db）
...
```sh
# 桌面 Python（uv；Windows 下用 --directory，--project 会踩 trampoline 路径问题）
uv run --directory markdown_util python -m pytest tests -q
...
- 链接语义改动：markdown_util 的 link_resolver 测试与安卓 `LinkResolverTest` 一起改——两端语义必须一致。
- zip 导出改动：`markdown_util/tests/test_file_browser.py` 的 plan_zip_bundle 系列。
- UI 改动：pytest 之外跑 CI 同款 boot 冒烟；纯布局微调可只靠编译通过。

## Conventions

- **媒体链接必须经 resolver**：存储形式 = `http://127.0.0.1:8765/images|assets/<name>` 绝对 URL；任何消费方（zip 导出、手机渲染）先 `parse` → `rewrite`，非本库 URL 一律原样透传。`markdown_util/link_resolver.py` 与 `android/.../resolver/LinkResolver.kt` 同语义，改一端必同步另一端及其测试。
...
- **长任务不占界面线程**：桌面走 `qtui/workers.py` 线程池 + 信号回 GUI；安卓走 `withContext(Dispatchers.IO)`，遍历类任务支持协程取消。
